...
const telemetryRollupService = require('./TelemetryRollupService');
...
const httpRequestClosureInvalid = (eid, batchSize = globalConfig.noOfEventsPerBatch) => {
    const events = new Array(batchSize).fill(1).map(_ => telemetryRollupService.generateInvalidEvents(eid))
...
const httpRequestClosureDuplicate = (eid, batchSize = globalConfig.noOfEventsPerBatch) => {
    const events = new Array(batchSize).fill(1).map(_ => telemetryRollupService.generateDuplicateEvents(eid))
    const body = { id: uuid.v4(), events: events };
    return () => ({
        sendEvent: sendEvent({ body }, globalConfig.pushViaApi, globalConfig.pushViaKafka),
        type: 'duplicate'
...
const httpRequestClosure = (eid, batchSize = globalConfig.noOfEventsPerBatch) => {
    const events = new Array(batchSize).fill(1).map(_ => telemetryRollupService.generateEvents(eid))
    const body = { id: uuid.v4(), events: events };
    return () => ({
        sendEvent: sendEvent({ body }, globalConfig.pushViaApi, globalConfig.pushViaKafka),
        type: 'valid'
