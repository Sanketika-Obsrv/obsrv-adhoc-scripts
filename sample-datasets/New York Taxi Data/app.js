...
for (let index = 1; index <= 5000; index++) {
  ...
  parsed = _.map(parsed, item => {
    let newItem =  {
      ...
      "fare_details": {
        "fare_amount": item.fare_details.fare_amount,
        "extra": item.fare_details.extra,
        "mta_tax": item.fare_details.mta_tax,
        "tip_amount": item.fare_details.tip_amount,
        "tolls_amount": item.fare_details.tolls_amount,
        "improvement_surcharge": item.fare_details.improvement_surcharge,
        "total_amount": item.fare_details.total_amount,
        "congestion_surcharge": item.fare_details.congestion_surcharge
      }
    }
    return newItem;
  })
